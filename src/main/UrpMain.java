...
public class UrpMain {
	...
	public void processInput() {
		Urp_tuples = dataset_inputs.get("source_urp").flatMap(
				new UrpFlatMap()).filter(new UrpFilter());
	...
	public void processAggregate() {
		...
		DataSet<Urp> urp_laccima4g_null = Urp_tuples
				...
				.where("LACCI_4g").equalTo("lacci")
				.with(new UrpLeftJoinLaccimaNULLOnly())
				.map(new TempUrpMap());
		...
		DataSet<Output> urp_mdl = urp_laccima4g_null_laccima3g_null 
				...
				.with(new UrpLeftJoinMDL()).flatMap(new Output2UrpFlatMap());

		output = urp_laccima4g.union(urp_laccima4g_null_laccima3g).union(urp_mdl).groupBy(0, 1, 2, 3, 4, 5, 6)
				.reduceGroup(new OutputGroupReducer("URP"));
...
//		 int sink_paralel = 1;
//		
//		 UrpMain main = new UrpMain(proses_paralel, sink_paralel, Constant.OUTPUT_URP);
